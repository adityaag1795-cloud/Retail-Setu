      ...
      <table class="table">
        ...
        <tbody>
          ${report.communications
            .map(
              (c: any) => `<tr><td>${escapeHtml(c.date)}</td><td>${escapeHtml(c.direction)}</td><td>${escapeHtml(c.channel)}${c.scanCopy ? " (scan)" : ""}</td><td>${escapeHtml(c.subject)}</td><td><a href="/api/outlets/${o.id}/communications/${c.id}/pdf" target="_blank">PDF</a></td></tr>`,
            )
...
async function renderCaseDetail(id: string) {
  ...
  if (c.stage === "Commissioned") {
    sections.push(
      `<h3>Outlet commissioned</h3><p><a href="#/outlets/${c.outletId}">View outlet record &rarr;</a> — canopy addition and further requests are managed from the outlet page.</p>`,
    );
  }
...
function renderCanopySection(outletId: string, outletStatus: string, req: any): string {
  ...
  if (!req.soDecision) {
    ...
    if (req.budgetNoteText) parts.push(`<h4>Budget note</h4><pre class="ai-output">${escapeHtml(req.budgetNoteText)}</pre>`);
...
function wireOutletCanopyHandlers(outletId: string) {
  const on = (sel: string, handler: (el: Element) => void) => {
    const el = document.querySelector(sel);
    if (el) handler(el);
  };

  on("#canopy-request-form", (el) =>
...
async function renderAnalytics() {
  ...
  app().innerHTML = `
    <section class="panel">
      ...
      <div class="grid-cards">
        <div class="card"><h3>Below TA average</h3><p class="big">${summary.belowTA.length}</p><ul>${summary.belowTA.map((x: any) => `<li>${escapeHtml(x.name)}: ${x.actualKL} / ${x.taAverageKL} KL</li>`).join("")}</ul></div>
  ...
  app().innerHTML = `
    <section class="panel">
      ...
      <ul>
        ...
        ${openWork.canopyProposalsAwaitingApproval.map((p: any) => `<li><a href="#/outlets/${p.outletId}">${escapeHtml(p.outletName)}</a> — ${escapeHtml(p.awaiting)}</li>`).join("")}
    ...
    <section class="panel">
      ...
      <table class="table">
        ...
        <tbody>${snap.calendar
          ...
          .map((e: any) => `<tr><td>${e.date}</td><td>${escapeHtml(e.type)}</td><td>${e.linkedCaseId ? `<a href="#/cases/${e.linkedCaseId}">${escapeHtml(e.title)}</a>` : escapeHtml(e.title)}</td><td>${escapeHtml(e.salesArea)}</td></tr>`)

...
class TemplateAiEngine implements AiEngine {
  ...
  private feasibilityReport(ctx: Record<string, unknown>): string {
    const { stretchName, competitorContext, roster } = ctx as {
      ...
      roster: { candidateName: string; location: string; feasible: boolean; remarks: string }[];
    };
    const feasibleCount = roster.filter((r) => r.feasible).length;
    const lines = roster
      .map((r, i) => `  ${i + 1}. ${r.candidateName} (${r.location}) — ${r.feasible ? "FEASIBLE" : "NOT FEASIBLE"}. ${r.remarks}`)
      .join("\n");
    return [
      `FEASIBILITY REPORT — ${stretchName}`,
      ``,
      `Competitive context: ${competitorContext}`,
      ``,
      `Candidate sites evaluated (${roster.length}), of which ${feasibleCount} assessed feasible:`,
      lines || "  (roster not yet populated)",
      ``,
      `Recommendation: Proceed to roster finalisation and application intake for feasible sites.`,
      ``,
      `[Generated by ${this.name} — replace with the business's actual feasibility report format when supplied.]`,
    ].join("\n");
  }

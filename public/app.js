...
function renderTrafficSection(traffic) {
    ...
    <table class="table">
      ...
      <tbody>
        ${traffic.nozzles
        .map((n) => `<tr><td>${escapeHtml(n.pumpNo)}</td><td>${escapeHtml(n.nozzleNo)}</td><td>${n.product ? escapeHtml(n.product) : '<span class="muted">not on file</span>'}</td><td>${n.transactionCount}</td><td>${n.lastTransactionAt.slice(0, 10)}</td><td>${n.possiblyInactive ? '<span class="badge badge--escalated">Possibly inactive</span>' : '<span class="badge badge--resolved">Active</span>'}</td></tr>`)
        .join("")}
      </tbody>
    </table>

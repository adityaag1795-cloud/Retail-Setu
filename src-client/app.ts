 ...
 */
function renderGrowthAnalysisSection(growth: { caveat: string; reports: any[] }): string {
  ...
  return `
    ...
    ${reports
      ...
      <div class="card">
        ...
        <p class="muted"><strong>Why (real transaction-slab trend):</strong>${
          g.hasTransactionLog
            ? ""
            : ` ${escapeHtml(g.slabNarrative[0])}`
        }</p>
        ...
        <p class="muted"><strong>DU uptime:</strong> ${
          g.duUptime
            ? `${g.duUptime.uptimePct}% (${g.duUptime.daysOnFile}/${g.duUptime.totalDays} days on file)${
                g.duUptime.gaps.length
                  ? " — gaps: " + g.duUptime.gaps.map((gap: any) => `${gap.startDate} to ${gap.endDate} (${gap.days}d)`).join(", ")
                  : ""
              }`

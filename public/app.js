 ...
 */
function renderHourlyProductChartSection(productHourly) {
 ...
 */
function renderTourCircuitSection(stops) {
    if (!stops.length) {
    ...
    <ul class="tour-compact">
      ${stops
        .map((s) => `<li><a href="#/outlets/${s.outletId}">${escapeHtml(s.outletName)}</a> — ${escapeHtml(s.reasons[0])}${s.reasons.length > 1 ? ` <span class="muted">(+${s.reasons.length - 1} more)</span>` : ""}${s.distanceFromPrevKm != null ? ` <span class="muted">&middot; ${s.distanceFromPrevKm} km</span>` : ""}</li>`)
        .join("")}
    </ul>`;
}
 ...
 */
function renderDistrictNewsSection(news) {
    if (news.ok) {
        return `<ul>${news.headlines.map((h) => `<li>${h.link ? `<a href="${escapeHtml(h.link)}" target="_blank" rel="noopener">${escapeHtml(h.title)}</a>` : escapeHtml(h.title)}</li>`).join("")}</ul><p class="muted">Live, ${escapeHtml(news.source)}</p>`;
    }
...
async function renderDealerDesk(momOutletId) {
    ...
    <section class="panel">
      ...
      <label>Outlet
        <select id="mom-outlet-select">
          <option value="">Select an outlet...</option>
          ${outlets.map((o) => `<option value="${o.id}" ${o.id === momOutletId ? "selected" : ""}>${escapeHtml(o.name)}</option>`).join("")}
        </select>
      </label>
      <div id="mom-section">
        ${momOutletId

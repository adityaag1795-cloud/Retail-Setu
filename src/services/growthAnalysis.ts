/**
 * Module 3 — per-outlet MS/HSD/Power volume growth or degrowth, year-over-year on the latest real
 * CY month on file (the same target=LY/achieved=CY basis as the KPI Tracker — see kpiTracker.ts),
 * paired with the real DU transaction-log signals (slab-wise mix trend, DU uptime) that can help
 * read WHY, where a transaction log actually exists for that outlet.
 *
...
export function outletVolumeGrowth(): OutletGrowthReport[] {
  const reports = store
    .visibleOutlets()
    .filter((o) => !!o.productComparison)
    .map((outlet) => {
      ...
      const products: OutletProductGrowth[] = GROWTH_PRODUCTS.map((label) => {
        ...
        const growthPct =
          last.target > 0 && last.achieved !== null ? Math.round(((last.achieved - last.target) / last.target) * 1000) / 10 : null;
  ...
  // Worst MS degrowth first — this is a decision-support list, so the outlets needing attention
  // should lead; outlets with no MS growth figure (shouldn't happen given the filter above, but
  // defensively) sort to the end rather than being treated as a worst-case 0.
  return reports.sort((a, b) => {
    const ap = a.products.find((p) => p.product === "MS (Petrol)")?.growthPct;
    const bp = b.products.find((p) => p.product === "MS (Petrol)")?.growthPct;

...
class TemplateAiEngine implements AiEngine {
  ...
  /** Initiation-stage remarks for the routing-chain file note that accompanies a modernisation-request decision. */
  private modernisationFileNote(ctx: Record<string, unknown>): string {
    ...
    const clauseLine = policyClauses.length
      ? `Basis ${policyClauses.map((c) => `${c.documentTitle} clause ${c.clauseNumber}`).join("; ")}, `
      : "";
    const irrLine = irr.irrPct === null ? "not viable within the chosen horizon" : `${irr.irrPct.toFixed(1)}% (${irr.meetsHurdle ? "meets" : "below"} the ${irr.minimumHurdlePct}% minimum per HQO circular)`;
    return (
      `Dealer at ${outletName} has requested ${modernisationType} modernisation. Dealer justification: ${dealerJustification}\n\n` +
      `SO recommendation: ${soJustification}\n\n` +
      `Cost estimate Rs. ${costEstimate.totalInvestment.toLocaleString("en-IN")}, IRR ${irrLine}. ${clauseLine}recommend approval and ` +
